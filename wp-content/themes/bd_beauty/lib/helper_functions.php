...
function AddGoogleFont($fontFamily,&$mthis){
	global $usedFonts;
	$Font=$mthis['config']->get($fontFamily);
	if(!empty($Font)){
		if(empty($usedFonts[$Font])){
			$FontUrl=str_replace(" ", "+", $Font);
			//$mthis['asset']->addFile('css', "$testFontUrl");
			$usedFonts[$Font]=$FontUrl;
		}
	}
}
function GetGoogleFont(&$mthis){
	global $usedFonts;
	$systemFont=array('Arial','tohama');	
	$http  = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
	$http = $http.'://fonts.googleapis.com/css?family=';
	$fontStr="";
	foreach($usedFonts as $font=>$value){
		if(in_array($font, $systemFont))continue;		
		$fontStr.=!empty($fontStr)?"|":"";
		$fontStr.=$value;
	}
	if(!empty($fontStr)){
		$mthis['asset']->addFile('css', $http.$fontStr);
	}
}
