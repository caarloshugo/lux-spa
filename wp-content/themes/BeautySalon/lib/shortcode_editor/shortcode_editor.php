...
class ShortcodeNinjaPlugin {
	...
	function action_admin_init() {
		if ( current_user_can( 'edit_posts' ) 
		  ...
		  && get_user_option('rich_editing') == 'true' )  {
			...
			/*
			// NOTE: including the woo styles is not needed anymore,
			//       because they are loaded by the preview php file
			$woo_shortcode_css = content_url() . '/themes/' 
			                                   . basename(get_template_directory())
			                                   . '/functions/css/shortcodes.css';
			                               
			wp_register_style('wooShortcodesStyles', $woo_shortcode_css);
			wp_enqueue_style('wooShortcodesStyles');
			*/
	 ...
	 */
	function plugin_url() {
		return (get_template_directory_uri().'/lib/shortcode_editor/');
