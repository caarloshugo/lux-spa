var scnDialogHelper = {
  ...
  loadShortcodeDetails: function () {
    if (scnSelectedShortcodeType) {
      var a = this;
      jQuery.getScript(bdt.template_url + "/lib/shortcode_editor/sc/" + scnSelectedShortcodeType + ".js", function () {
        a.initializeDialog()
      })
    }
  },
  ...
  createSelectControl: function (a, b, c) { // EDITED
    var f = a.validateLink ? "scn-validation-marker" : "",
        d = a.isRequired ? "scn-required" : "",
        g = "scn-value-" + a.id;
    var selectField = jQuery('<select>').attr("id", g).attr("name", g).addClass(f).addClass(d).addClass('select').addClass(c ? "scn-focus-here" : "");
    b.addClass('scn-marker-select-control');
    for(i in a.options) {
      var label = a.options[i];
      var value = a.options[i];
      var selected = '';
      if (value == a.defValue) {
        label = a.defText;
        selected = ' selected="selected"';
      }
      selectField.append('<option value="' + value + '"' + selected + '>' + label + '</option>');
    }
    selectField.appendTo(b);
    if(a = a.help) {
      jQuery("<br/>").appendTo(b);
      jQuery("<span/>").addClass("scn-input-help").html(a).appendTo(b)
    }
  ...
  getTextKeyValue: function (a) {
    var b = a.find("input");
    if (!b.length || b.attr('type') == 'button') return null; // EDITED
  ...
  getSelectKeyValue: function (a) { // EDITED
    var b = a.find("select");
  ...
  previewAction: function (a) {
    ...
    jQuery("#scn-preview-iframe").attr("src", bdt.template_url+"/lib/shortcode_editor/preview-shortcode-external.php?shortcode=" + encodeURIComponent(this.makeShortcode()))
  },
