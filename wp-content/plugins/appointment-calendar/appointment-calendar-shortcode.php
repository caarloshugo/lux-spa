...
<!------------- Modal Form Works -------------------->
		...
		jQuery('#booknowapp').click(function(){
			...
			if(!start_time)
			{
				jQuery("#selecttimediv").after("<span class='error'><br><strong>Select any time.</strong></span>");
				return false;  
			}
...
var SecondData = "ServiceId=" + ServiceId + "&AppDate=" + AppDate + "&StartTime=" + StartTime + '&Client_Name=' + Client_Name +'&Client_Email=' + Client_Email +'&Client_Phone=' + Client_Phone +'&Client_Note=' + Client_Note+'&Service_Duration=' + ServiceDuration; 
	...
	<div id="bkbtndiv" align="center" style="padding:5px;">
		...
		<?php 
			...
			else echo _e("Schedule New Appointment");
		?>
		</button>
	</div>
		...
		<div class="modal-body">
			...
			<?php
				 ...
				 *************************/
					...
					{
						echo "<div class='alert alert-info'>Available Time For <strong>'$ServiceData->name'</strong> On <strong>'".date("l, jS F Y", strtotime($AppointmentDate))."'</strong></div>";
			...
			{
				echo "<p align=center class='alert alert-error' style='width:auto;'><strong>Sorry! Today's all appointments has been booked.</strong></p>";
				echo '<a class="btn btn-primary" id="back" onclick="Backbutton()">&larr; Back</a>';
			}
